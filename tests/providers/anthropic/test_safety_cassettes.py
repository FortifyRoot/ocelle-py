"""
SDK safety cassette tests for Anthropic (Phase T5-B).

End-to-end tests through the vendored code path:
  fortifyroot.init() -> Anthropic API call (VCR) -> safety masking -> spans
...
Capability matrix (SAFETY_ARCHITECTURE.md):
  Anthropic -- sync, async, stream, non-text (vision)
...
# ---------------------------------------------------------------------------
# Minimal base64-encoded 1x1 PNG for vision tests
# ---------------------------------------------------------------------------

# Valid 10x10 red PNG (Anthropic rejects 1x1 images)
TINY_PNG = (
...
def _init_with_safety(init_provider_sdk, action="MASK"):
    ...
    init_provider_sdk(instruments={Instruments.ANTHROPIC})
    register_all_handlers()
...
def _assert_telemetry(span, *, check_tokens=False, span_name="anthropic.chat"):
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_sync_prompt_mask_credit_card(
    pytestconfig, init_provider_sdk, span_exporter
):
    """Sync: credit card in prompt -> masked."""
    cassette = "test_anthropic_safety_sync_prompt_mask_credit_card"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_sync_prompt_mask_list_rule(
    pytestconfig, init_provider_sdk, span_exporter
):
    """Sync: competitor org names (list rule) -> masked."""
    cassette = "test_anthropic_safety_sync_prompt_mask_list_rule"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_sync_prompt_mask_combined(
    pytestconfig, init_provider_sdk, span_exporter
):
    """Sync: all rule types fire in a single prompt (RegEx + List)."""
    cassette = "test_anthropic_safety_sync_prompt_mask_combined"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_sync_completion_mask_email(
    pytestconfig, init_provider_sdk, span_exporter
):
    ...
    """
    cassette = "test_anthropic_safety_sync_completion_mask_email"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_sync_prompt_allow_email(
    pytestconfig, init_provider_sdk, span_exporter
):
    """ALLOW: email detected but text passes through unchanged. Findings recorded."""
    cassette = "test_anthropic_safety_sync_prompt_allow_email"
    ensure_key_or_cassette(pytestconfig, cassette)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_sync_prompt_allow_credit_card(
    pytestconfig, init_provider_sdk, span_exporter
):
    """ALLOW: credit card detected but text passes through unchanged."""
    cassette = "test_anthropic_safety_sync_prompt_allow_credit_card"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk, action=SAFETY_ACTION_ALLOW)
...
@pytest.mark.fr
@pytest.mark.asyncio
async def test_anthropic_safety_async_prompt_mask_email(
    pytestconfig, init_provider_sdk, span_exporter
):
    """Async: email in prompt -> masked. Verify telemetry + tokens."""
    cassette = "test_anthropic_safety_async_prompt_mask_email"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
...
@pytest.mark.fr
@pytest.mark.asyncio
async def test_anthropic_safety_async_completion_mask_email(
    pytestconfig, init_provider_sdk, span_exporter
):
    ...
    """
    cassette = "test_anthropic_safety_async_completion_mask_email"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_stream_prompt_mask_email(
    pytestconfig, init_provider_sdk, span_exporter
):
    """Streaming: email in prompt -> masked. Verify streaming span."""
    cassette = "test_anthropic_safety_stream_prompt_mask_email"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_stream_completion_mask(
    pytestconfig, init_provider_sdk, span_exporter
):
    """Streaming: verify stream completes, prompt masked, completion captured.

    Note: Streaming completion masking relies on the holdback algorithm. VCR
    ...
    """
    cassette = "test_anthropic_safety_stream_completion_mask"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_no_config_passthrough(
    pytestconfig, init_provider_sdk, span_exporter
):
    """No safety handlers -> text passes through unchanged, telemetry present."""
    cassette = "test_anthropic_safety_no_config_passthrough"
    ensure_key_or_cassette(pytestconfig, cassette)
...
@pytest.mark.vcr
@pytest.mark.fr
def test_anthropic_safety_vision_skips_image(
    pytestconfig, init_provider_sdk, span_exporter
):
    """Vision: image content untouched, text part masked, telemetry captured."""
    cassette = "test_anthropic_safety_vision_skips_image"
    ensure_key_or_cassette(pytestconfig, cassette)
    _init_with_safety(init_provider_sdk)
    ...
    try:
        ...
        msg = str(err).lower()
        if "image" in msg and ("format" in msg or "invalid" in msg or "decode" in msg):
    ...
    # Multi-part content (text + image) may not be stored in the standard
    # prompt attribute. Check for either masked prompt or safety events.
    prompt = _prompt_content(span)
    has_masked = prompt is not None and "[PII.email]" in prompt
    events = _safety_events(span)
    has_events = len(events) >= 1

    assert has_masked or has_events, (
        "Vision test: expected either masked prompt or safety events"
    )
...
@pytest.mark.vcr
@pytest.mark.fr
@pytest.mark.default_cassette("test_anthropic_safety_sync_prompt_mask_email")
def test_anthropic_fortifyroot_attributes(
    pytestconfig, init_provider_sdk, span_exporter
):
