...
from fortifyroot._vendor.opentelemetry.instrumentation.openai.shared import (
    _set_span_attribute,
    model_as_dict,
)
...
from fortifyroot._vendor.opentelemetry.instrumentation.openai.shared.event_models import (
    ChoiceEvent,
    MessageEvent,
)
...
from fortifyroot._vendor.opentelemetry.instrumentation.openai.v1.assistant_safety import (
...
from fortifyroot._vendor.opentelemetry.semconv_ai import LLMRequestTypeValues, SpanAttributes
from opentelemetry.trace import SpanKind, Status, StatusCode


@_with_tracer_wrapper
async def aassistants_create_wrapper(tracer, wrapped, instance, args, kwargs):
...
@_with_tracer_wrapper
async def amessages_create_wrapper(tracer, wrapped, instance, args, kwargs):
    if context_api.get_value(_SUPPRESS_INSTRUMENTATION_KEY):
        return await wrapped(*args, **kwargs)
...
@_with_tracer_wrapper
async def aruns_create_wrapper(tracer, wrapped, instance, args, kwargs):
    if context_api.get_value(_SUPPRESS_INSTRUMENTATION_KEY):
        return await wrapped(*args, **kwargs)
...
@_with_tracer_wrapper
async def aruns_retrieve_wrapper(tracer, wrapped, instance, args, kwargs):
    @dont_throw
...
@_with_tracer_wrapper
async def amessages_list_wrapper(tracer, wrapped, instance, args, kwargs):
    if context_api.get_value(_SUPPRESS_INSTRUMENTATION_KEY):
        return await wrapped(*args, **kwargs)
        ...
        if assistants.get(run["assistant_id"]) is not None or Config.enrich_assistant:
            if Config.enrich_assistant:
                assistant = model_as_dict(
                    await instance._client.beta.assistants.retrieve(run["assistant_id"])
                )
...
@_with_tracer_wrapper
async def aruns_create_and_stream_wrapper(tracer, wrapped, instance, args, kwargs):
    if context_api.get_value(_SUPPRESS_INSTRUMENTATION_KEY):
        return await wrapped(*args, **kwargs)
    ...
    with trace.use_span(span, end_on_exit=False):
        ...
        if assistants.get(assistant_id) is not None or Config.enrich_assistant:
            if Config.enrich_assistant:
                assistant = model_as_dict(
                    await instance._client.beta.assistants.retrieve(assistant_id)
                )
        ...
        from fortifyroot._vendor.opentelemetry.instrumentation.openai.v1.event_handler_wrapper import (
            EventHandleWrapper,
        )
