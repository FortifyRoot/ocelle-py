...
from fortifyroot._vendor.opentelemetry.instrumentation.together.event_models import (
    CompletionEvent,
    PromptEvent,
)
from fortifyroot._vendor.opentelemetry.instrumentation.together.utils import (
    dont_throw,
    should_emit_events,
    should_send_prompts,
)
...
def emit_event(
    event: Union[PromptEvent, CompletionEvent], event_logger: Union[Logger, None]
) -> None:
    """
...
def _emit_message_event(event: PromptEvent, event_logger: Logger) -> None:
    body = asdict(event)
...
def _emit_choice_event(event: CompletionEvent, event_logger: Logger) -> None:
    body = asdict(event)
