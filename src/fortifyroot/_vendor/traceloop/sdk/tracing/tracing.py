...
from fortifyroot._vendor.traceloop.sdk.images.image_uploader import ImageUploader
from fortifyroot._vendor.traceloop.sdk.instruments import Instruments
...
from fortifyroot._vendor.traceloop.sdk.tracing.content_allow_list import ContentAllowList
from fortifyroot._vendor.traceloop.sdk.utils import is_notebook
from fortifyroot._vendor.traceloop.sdk.utils.package_check import is_package_installed
...
def _set_association_properties_attributes(span, properties: dict) -> None:
    for key, value in properties.items():
        span.set_attribute(
            f"{SpanAttributes.FORTIFYROOT_ASSOCIATION_PROPERTIES}.{key}", value
        )
...
def default_span_processor_on_start(span: Span, parent_context: Context | None = None):
    ...
    if workflow_name is not None:
        span.set_attribute(SpanAttributes.FORTIFYROOT_WORKFLOW_NAME, str(workflow_name))
    ...
    if entity_path is not None:
        span.set_attribute(SpanAttributes.FORTIFYROOT_ENTITY_PATH, str(entity_path))
    ...
    if is_llm_span(span):
        ...
        if managed_prompt is not None:
            span.set_attribute(
                SpanAttributes.FORTIFYROOT_PROMPT_MANAGED, str(managed_prompt)
            )

        prompt_key = get_value("prompt_key")
        if prompt_key is not None:
            span.set_attribute(SpanAttributes.FORTIFYROOT_PROMPT_KEY, str(prompt_key))

        prompt_version = get_value("prompt_version")
        if prompt_version is not None:
            span.set_attribute(
                SpanAttributes.FORTIFYROOT_PROMPT_VERSION, str(prompt_version)
            )

        prompt_version_name = get_value("prompt_version_name")
        if prompt_version_name is not None:
            span.set_attribute(
                SpanAttributes.FORTIFYROOT_PROMPT_VERSION_NAME, str(prompt_version_name)
            )

        prompt_version_hash = get_value("prompt_version_hash")
        if prompt_version_hash is not None:
            span.set_attribute(
                SpanAttributes.FORTIFYROOT_PROMPT_VERSION_HASH, str(prompt_version_hash)
            )

        prompt_template = get_value("prompt_template")
        if prompt_template is not None:
            span.set_attribute(
                SpanAttributes.FORTIFYROOT_PROMPT_TEMPLATE, str(prompt_template)
            )
        ...
        if prompt_template_variables is not None and isinstance(
            ...
            for key, value in prompt_template_variables.items():
                span.set_attribute(
                    f"{SpanAttributes.FORTIFYROOT_PROMPT_TEMPLATE_VARIABLES}.{key}",
                    value,
                )
...
def init_cohere_instrumentor():
    try:
        if is_package_installed("cohere"):
            from fortifyroot._vendor.opentelemetry.instrumentation.cohere import CohereInstrumentor
...
def init_pinecone_instrumentor():
    try:
        if is_package_installed("pinecone"):
            from fortifyroot._vendor.opentelemetry.instrumentation.pinecone import PineconeInstrumentor
...
def init_qdrant_instrumentor():
    try:
        if is_package_installed("qdrant_client") or is_package_installed(
            ...
            from fortifyroot._vendor.opentelemetry.instrumentation.qdrant import QdrantInstrumentor
...
def init_chroma_instrumentor():
    try:
        if is_package_installed("chromadb"):
            from fortifyroot._vendor.opentelemetry.instrumentation.chromadb import ChromaInstrumentor
...
def init_haystack_instrumentor():
    try:
        if is_package_installed("haystack"):
            from fortifyroot._vendor.opentelemetry.instrumentation.haystack import HaystackInstrumentor
...
def init_mistralai_instrumentor():
    try:
        if is_package_installed("mistralai"):
            from fortifyroot._vendor.opentelemetry.instrumentation.mistralai import MistralAiInstrumentor
...
def init_ollama_instrumentor():
    try:
        if is_package_installed("ollama"):
            from fortifyroot._vendor.opentelemetry.instrumentation.ollama import OllamaInstrumentor
...
def init_transformers_instrumentor():
    try:
        if is_package_installed("transformers"):
            from fortifyroot._vendor.opentelemetry.instrumentation.transformers import (
...
def init_together_instrumentor():
    try:
        if is_package_installed("together"):
            from fortifyroot._vendor.opentelemetry.instrumentation.together import TogetherAiInstrumentor
...
def init_milvus_instrumentor():
    try:
        if is_package_installed("pymilvus"):
            from fortifyroot._vendor.opentelemetry.instrumentation.milvus import MilvusInstrumentor
...
def init_sagemaker_instrumentor(should_enrich_metrics: bool):
    try:
        if is_package_installed("boto3"):
            from fortifyroot._vendor.opentelemetry.instrumentation.sagemaker import SageMakerInstrumentor
...
def init_replicate_instrumentor():
    try:
        if is_package_installed("replicate"):
            from fortifyroot._vendor.opentelemetry.instrumentation.replicate import ReplicateInstrumentor
...
def init_vertexai_instrumentor(
    ...
    try:
        if is_package_installed("google-cloud-aiplatform"):
            from fortifyroot._vendor.opentelemetry.instrumentation.vertexai import VertexAIInstrumentor
...
def init_weaviate_instrumentor():
    try:
        if is_package_installed("weaviate"):
            from fortifyroot._vendor.opentelemetry.instrumentation.weaviate import WeaviateInstrumentor
...
def init_writer_instrumentor():
    try:
        if is_package_installed("writer-sdk"):
            from fortifyroot._vendor.opentelemetry.instrumentation.writer import WriterInstrumentor
...
def init_agno_instrumentor():
    try:
        if is_package_installed("agno"):
            from fortifyroot._vendor.opentelemetry.instrumentation.agno import AgnoInstrumentor
...
def init_alephalpha_instrumentor():
    try:
        if is_package_installed("aleph_alpha_client"):
            from fortifyroot._vendor.opentelemetry.instrumentation.alephalpha import AlephAlphaInstrumentor
...
def init_marqo_instrumentor():
    try:
        if is_package_installed("marqo"):
            from fortifyroot._vendor.opentelemetry.instrumentation.marqo import MarqoInstrumentor
...
def init_lancedb_instrumentor():
    try:
        if is_package_installed("lancedb"):
            from fortifyroot._vendor.opentelemetry.instrumentation.lancedb import LanceInstrumentor
...
def init_groq_instrumentor():
    try:
        if is_package_installed("groq"):
            from fortifyroot._vendor.opentelemetry.instrumentation.groq import GroqInstrumentor
...
def init_crewai_instrumentor():
    try:
        if is_package_installed("crewai"):
            ...
            from fortifyroot._vendor.opentelemetry.instrumentation.crewai import CrewAIInstrumentor
...
def init_mcp_instrumentor():
    try:
        if is_package_installed("mcp"):
            from fortifyroot._vendor.opentelemetry.instrumentation.mcp import McpInstrumentor
...
def init_openai_agents_instrumentor():
    try:
        if is_package_installed("openai-agents"):
            from fortifyroot._vendor.opentelemetry.instrumentation.openai_agents import (
...
def metrics_common_attributes():
    ...
    workflow_name = get_value("workflow_name")
    if workflow_name is not None:
        common_attributes[SpanAttributes.FORTIFYROOT_WORKFLOW_NAME] = workflow_name

    entity_name = get_value("entity_name")
    if entity_name is not None:
        common_attributes[SpanAttributes.FORTIFYROOT_ENTITY_NAME] = entity_name

    association_properties = get_value("association_properties")
    if association_properties is not None:
        for key, value in association_properties.items():
            common_attributes[
                f"{SpanAttributes.FORTIFYROOT_ASSOCIATION_PROPERTIES}.{key}"
            ] = value
